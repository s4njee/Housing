...
model1 <- lm(log(sale.price.n) ~ log(gross.sqft),data=mp.homes)

mp.homes[which(mp.homes$gross.sqft==0),]
mp.homes <- mp.homes[which(mp.homes$gross.sqft>0 & mp.homes$land.sqft>0),]
model1 <- lm(log(sale.price.n) ~ log(gross.sqft), data=mp.homes)
summary(model1)
...
summary(model2)
png("analysis/model2.png")
plot(resid(model2))
dev.off()

model2a<- lm(log(sale.price.n) ~ 0 +log(gross.sqft) + log(land.sqft) + factor(neighborhood),data=mp.homes)
summary(model2a)
png("analysis/model2a.png")
plot(resid(model2a))
dev.off()

model3<- lm(log(sale.price.n) ~ log(gross.sqft)+ log(land.sqft)+ factor(neighborhood)+ factor(building.class.category),data=mp.homes)
summary(model3)
png("analysis/model3.png")
plot(resid(model3))
dev.off()

model4 <- lm(log(sale.price.n) ~ log(gross.sqft)+ log(land.sqft)+ factor(neighborhood)+ factor(building.class.category),data=mp.homes)
summary(model4)
png("analysis/model4.png")
plot(resid(model4))
dev.off()
